...
Project Chrono (BSD-3-Clause) is the primary capability and validation reference for the multibody family. Public documentation was used to identify the architectural separation between rigid bodies/links, time steppers and smooth-vs-nonsmooth contact formulations. cfd_solvers does not copy Chrono's class hierarchy or implementation: v0.16.x independently implements quaternion rigid bodies, Jacobian-row projected constraints, convex/mesh collision, smooth and nonsmooth contact, history-bearing DEM, resident/distributed DEM execution, conservative CFD coupling and advanced multibody integration.

v0.16.8 adds `ImplicitGeneralizedIntegrator`, an independently implemented dense nonlinear generalized-coordinate Newmark/generalized-alpha/HHT baseline; `ArticulatedSystem`, which assembles a fixed-base tree mass matrix from body Jacobians for revolute/prismatic coordinates; and `FlexibleBodyInterface`, which conservatively maps floating-frame/modal interface motion and FEM nodal reactions. The articulated baseline is deliberately readable O(n^3) dense dynamics rather than a claim of Featherstone ABA parity. The only remaining Phase-13 tracker item is real multi-rank qualification of distributed-memory DEM.


Phase 14 starts in v0.17.0 with `include/cfd/spectral/fft.hpp`, `src/spectral/fft.cpp`, `include/cfd/acoustics/kspace2d.hpp`, `src/acoustics/kspace2d.cpp`, and the acoustic post-processing helpers. The implementation is clean-room and uses public k-Wave method descriptions only as a capability/validation reference.
