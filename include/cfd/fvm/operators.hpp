...
[[nodiscard]] std::vector<double> orthogonal_laplacian_scalar(const PolyMesh& mesh,
                                                               ...
                                                               std::span<const double> boundary_face_values = {});
[[nodiscard]] std::vector<Vec3> least_squares_gradient_scalar(const PolyMesh& mesh,
    std::span<const double> cell_values,std::span<const double> boundary_face_values = {});
[[nodiscard]] std::vector<double> corrected_face_normal_gradient_scalar(const PolyMesh& mesh,
    std::span<const double> cell_values,std::span<const double> boundary_face_values = {});
[[nodiscard]] std::vector<double> corrected_laplacian_scalar(const PolyMesh& mesh,
    std::span<const double> cell_values,double diffusivity = 1.0,
    std::span<const double> boundary_face_values = {});
