...
[[nodiscard]] RealVec3 nedelec_tet4_basis(const NedelecTet4Geometry& geometry,
                                          std::size_t local_edge,
                                          std::array<double,4> barycentric);
[[nodiscard]] RealVec3 nedelec_tet4_curl(const NedelecTet4Geometry& geometry,
                                         std::size_t local_edge);
...
[[nodiscard]] EdgeMaxwell3DResult solve_driven_edge_maxwell_3d(
    const cfd::fem::Mesh3D& mesh,
    const EdgeMaxwell3DConfig& config,
...
[[nodiscard]] ResonatorQuality3D resonator_quality_3d(
    const cfd::fem::Mesh3D& mesh,
    const EdgeMaxwell3DConfig& config,
