...
[[nodiscard]] double sar_from_rms_electric_field(double conductivity_s_per_m, double tissue_density_kg_per_m3,
...
struct TemperatureFeedbackConfig {
    double reference_temperature_k{310.15};
    double conductivity_reference_s_per_m{1.0};
    double conductivity_coefficient_per_k{0.01};
    double perfusion_reference_per_s{0.01};
    double perfusion_coefficient_per_k{0.02};
};

struct TemperatureFeedbackValues {
    double conductivity_s_per_m{};
    double perfusion_per_s{};
};

[[nodiscard]] TemperatureFeedbackValues temperature_feedback(const TemperatureFeedbackConfig& config,
                                                             double temperature_k);

enum class ExposureScenario { implant, wearable };

struct BioelectromagneticExposureCase {
    std::string name;
    ExposureScenario scenario{ExposureScenario::wearable};
    VoxelTissueGrid3D grid;
    double averaging_mass_kg{0.01};
    double limit_w_per_kg{2.0};
};

struct ExposureValidationResult {
    double maximum_mass_averaged_sar_w_per_kg{};
    bool within_limit{};
};

[[nodiscard]] ExposureValidationResult validate_exposure_case(const BioelectromagneticExposureCase& exposure);

[[nodiscard]] std::vector<double> voxel_sar_w_per_kg(const VoxelTissueGrid3D& grid);
[[nodiscard]] double max_mass_averaged_sar_w_per_kg(const VoxelTissueGrid3D& grid, std::span<const double> sar_w_per_kg,
                                                    double averaging_mass_kg);
[[nodiscard]] std::vector<double> project_voxel_sar_to_pennes2d(const VoxelTissueGrid3D& grid,
                                                                std::span<const double> sar_w_per_kg, std::size_t nx,
                                                                std::size_t ny);
...
public:
    ...
    [[nodiscard]] const std::vector<double>& volumetric_heating_w_per_m3() const noexcept {
        return volumetric_heating_;
    }
