...
struct ScalarBoundaryCondition {
    ScalarBoundaryType type{ScalarBoundaryType::zeroGradient};
    double value{};
};

struct ScalarTransportConfig {
    double dt{1.0e-3};
    double diffusivity{1.0e-2};
    std::size_t linear_iterations{400};
    std::size_t gmres_restart{30};
    double linear_tolerance{1.0e-10};
};

// Implicit first-order-upwind advection + orthogonal diffusion on PolyMesh.
// This is the conservative baseline transport equation that later energy,
// species and electrochemistry modules can share. Higher-order deferred
// corrections can be layered on top without changing the sparse operator API.
...
private:
    PolyMesh mesh_;
    ScalarTransportConfig config_;
    std::vector<ScalarBoundaryCondition> boundary_;
    std::vector<double> values_;
    std::vector<double> old_values_;
    std::vector<double> source_;
    std::vector<double> face_flux_;
    cfd::core::CsrMatrix matrix_;
    std::optional<cfd::core::Ilu0Preconditioner> preconditioner_;
    std::vector<double> boundary_rhs_,rhs_,candidate_;
    bool operator_dirty_{true};
    std::size_t operator_assemblies_{};
