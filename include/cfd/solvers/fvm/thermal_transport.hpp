...
public:
    ...
    void set_volumetric_source(std::function<double(Vec3,double)> source){
        if(!source){source_={};return;} source_=[fn=std::move(source)](Vec3 x,double time,double){return fn(x,time);};
    }
    void set_temperature_dependent_source(std::function<double(Vec3,double,double)> source){source_=std::move(source);}
    void set_reactive_radiative_source(std::function<double(Vec3,double,double)> chemistry_heat_release,
                                       std::shared_ptr<const RadiationSourceModel> radiation,
                                       double environment_temperature);
    void set_material(std::function<double(Vec3,double)> heat_capacity,
                      std::function<double(Vec3,double)> conductivity);
...
private:PolyMesh mesh_;ThermalTransportConfig config_;std::vector<double> temperature_,next_,mass_flux_;std::vector<ThermalBoundary> boundary_;std::function<double(Vec3,double,double)> source_;std::function<double(Vec3,double)> cp_,k_;double time_{};
};
