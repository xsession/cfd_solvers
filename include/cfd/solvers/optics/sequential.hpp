...
struct SequentialSurface {
    ...
    double vertex_z{};
    double radius{};             // signed sphere radius; ignored for plane
    double aperture_radius{1.0e30};
    double refractive_index_after{1.0};
    double conic_constant{};
    std::array<double,4> even_coefficients{}; // r^4, r^6, r^8, r^10 sag terms
};
...
private:
    double object_space_index_{};
    std::vector<SequentialSurface> surfaces_;
};

[[nodiscard]] double rms_spot_radius_at_plane(const std::vector<RayTraceResult>& rays,
                                              double image_z);
