...
[[nodiscard]] std::vector<double> q_criterion_3d(std::size_t nx,std::size_t ny,std::size_t nz,std::span<const Vector3> velocity,double dx=1.0,double dy=1.0,double dz=1.0);
...
void write_html_scalar_viewer(const std::filesystem::path& path,std::size_t nx,std::size_t ny,std::span<const double> field);
} // namespace cfd::lbm
