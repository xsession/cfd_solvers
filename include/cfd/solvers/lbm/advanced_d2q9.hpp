...
class CumulantD2Q9Solver {
public:explicit CumulantD2Q9Solver(AdvancedD2Q9Config config={});void initialize_uniform(float rho=1,float ux=0,float uy=0);void initialize_taylor_green(float amplitude=.03F);void step(std::size_t count=1);[[nodiscard]] double mass()const;[[nodiscard]] double kinetic_energy()const;
private:AdvancedD2Q9Config c_;cfd::core::Grid2D grid_;cfd::core::StaticSoA<float,9>f_,next_;std::array<std::array<double,9>,9>raw_inv_{};void step_once();};
class MrtD2Q9Solver {
