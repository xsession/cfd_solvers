...
void validate_solution(std::span<const int> charge, std::span<const double> molality) {
    if (charge.size() != molality.size()) throw std::invalid_argument("activity-model size mismatch");
    for (double m : molality) if (!(m >= 0.0) || !std::isfinite(m)) throw std::invalid_argument("invalid molality");
}
...
double Nasa7Polynomial::h_molar(double t) const {
    validate_temperature(t, minimum_temperature, maximum_temperature);
    const double h_rt = a[0] + a[1]*t/2.0 + a[2]*t*t/3.0 + a[3]*t*t*t/4.0
                      + a[4]*t*t*t*t/5.0 + a[5]/t;
    return gas_constant * t * h_rt;
}
double Nasa7Polynomial::s_molar(double t) const {
    validate_temperature(t, minimum_temperature, maximum_temperature);
    const double s_r = a[0]*std::log(t) + a[1]*t + a[2]*t*t/2.0 + a[3]*t*t*t/3.0
                     + a[4]*t*t*t*t/4.0 + a[6];
    return gas_constant * s_r;
}

std::vector<double> DaviesActivity::activity_coefficients(
    std::span<const int> charge, std::span<const double> molality, double temperature) const {
    validate_solution(charge, molality);
    if (!(temperature > 0.0) || !std::isfinite(temperature) || !(a_ > 0.0)) throw std::invalid_argument("invalid Davies controls");
...
std::vector<double> SitActivity::activity_coefficients(
    std::span<const int> charge, std::span<const double> molality, double temperature) const {
    validate_solution(charge, molality);
    if (!(temperature > 0.0) || !std::isfinite(temperature) || !(a_ > 0.0)) throw std::invalid_argument("invalid SIT controls");
    const double i = ionic_strength(charge, molality);
    const double root = std::sqrt(i);
    ...
    for (const auto& pair : interactions_) {
        ...
        if (pair.first != pair.second) log10_gamma[pair.second] += pair.epsilon * molality[pair.first];
    }
    std::vector<double> gamma(log10_gamma.size());
    for (std::size_t k = 0; k < gamma.size(); ++k) gamma[k] = std::pow(10.0, log10_gamma[k]);
    return gamma;
}
...
BinaryIdealEquilibrium minimize_binary_ideal_gibbs(double mu_a, double mu_b, double t) {
    ...
    double xb{};
    if (delta > 50.0) xb = std::exp(-delta);
    else if (delta < -50.0) xb = 1.0 - std::exp(delta);
    else xb = 1.0 / (1.0 + std::exp(delta));
...
double binary_ion_exchange_fraction(double selectivity, double activity_a, double activity_b) {
    if (!(selectivity > 0.0) || !(activity_a >= 0.0) || !(activity_b >= 0.0)
        || !std::isfinite(selectivity) || !std::isfinite(activity_a) || !std::isfinite(activity_b)) {
        throw std::invalid_argument("invalid ion-exchange controls");
    }
    if (activity_b == 0.0) return 0.0;
    if (activity_a == 0.0) return 1.0;
    const double ratio = selectivity * activity_b / activity_a;
    return ratio / (1.0 + ratio);
}


double ShomatePolynomial::cp_molar(double T) const{if(!(T>=minimum_temperature&&T<=maximum_temperature))throw std::out_of_range("Shomate temperature out of range");double t=T/1000.0;return A+B*t+C*t*t+D*t*t*t+E/(t*t);}
double ShomatePolynomial::h_molar_kj(double T) const{if(!(T>=minimum_temperature&&T<=maximum_temperature))throw std::out_of_range("Shomate temperature out of range");double t=T/1000.0;return A*t+B*t*t/2+C*t*t*t/3+D*t*t*t*t/4-E/t+F-H;}
double ShomatePolynomial::s_molar(double T) const{if(!(T>=minimum_temperature&&T<=maximum_temperature))throw std::out_of_range("Shomate temperature out of range");double t=T/1000.0;return A*std::log(t)+B*t+C*t*t/2+D*t*t*t/3-E/(2*t*t)+G;}
double BinaryPitzer11::mean_activity_coefficient(double m,double T) const{if(!(m>=0)||!(T>0))throw std::invalid_argument("invalid Pitzer state");if(m==0)return 1.0;const double I=m,s=std::sqrt(I),Aphi=0.392,b=1.2,alpha=2.0,x=alpha*s;double g=x<1e-8?1.0:2.0*(1.0-(1.0+x)*std::exp(-x))/(x*x);double f=-Aphi*(s/(1.0+b*s)+(2.0/b)*std::log(1.0+b*s));double ln_gamma=f+m*(2.0*beta0+2.0*beta1*g)+1.5*m*m*c_phi;return std::exp(ln_gamma);}
