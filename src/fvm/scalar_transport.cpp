...
ScalarTransport::ScalarTransport(PolyMesh mesh,ScalarTransportConfig config)
    :mesh_(std::move(mesh)),config_(config),boundary_(mesh_.patches().size()),values_(mesh_.cell_count(),0.0),
     ...
     boundary_rhs_(mesh_.cell_count(),0.0),rhs_(mesh_.cell_count(),0.0),candidate_(mesh_.cell_count(),0.0) {
...
void ScalarTransport::initialize(double value){std::fill(values_.begin(),values_.end(),value);old_values_=values_;time_=0.0;}
void ScalarTransport::initialize(const std::function<double(Vec3)>& value){for(std::size_t c=0;c<mesh_.cell_count();++c)values_[c]=value(mesh_.cells()[c].center);old_values_=values_;time_=0.0;}
...
void ScalarTransport::assemble_operator(){
    ...
    for(std::size_t cell=0;cell<n;++cell){const double volume=mesh_.cells()[cell].volume;double ap=volume/config_.dt,b=0.0;
    ...
    matrix_=builder.build();preconditioner_.emplace(matrix_);operator_dirty_=false;++operator_assemblies_;
}
cfd::core::IterativeSolverResult ScalarTransport::step(){if(operator_dirty_)assemble_operator();old_values_=values_;candidate_=values_;
    for(std::size_t c=0;c<mesh_.cell_count();++c){const double v=mesh_.cells()[c].volume;rhs_[c]=old_values_[c]*(v/config_.dt)+source_[c]*v+boundary_rhs_[c];}
