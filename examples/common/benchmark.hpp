...
inline void emit(const BenchmarkRecord& r) {
    ...
    std::cout << std::setprecision(17) << "CFD_BENCH {" << "\"schema\":1," << "\"version\":\"" << CFD_SOLVERS_VERSION
              << "\"," << "\"feature_set\":\"" << r.feature_set << "\"," << "\"case\":\"" << r.case_name << "\","
              << "\"backend\":\"" << r.backend << "\"," << "\"problem_size\":" << r.problem_size << ','
              << "\"steps\":" << r.steps << ',' << "\"setup_ms\":" << r.setup_ms << ','
              << "\"simulation_ms\":" << r.simulation_ms << ',' << "\"total_ms\":" << (r.setup_ms + r.simulation_ms)
              << ',' << "\"work_units\":" << r.work_units << ',' << "\"work_unit_name\":\"" << r.unit << "\","
              << "\"throughput\":" << throughput << ',' << "\"checksum\":" << r.checksum << "}\n";
}
