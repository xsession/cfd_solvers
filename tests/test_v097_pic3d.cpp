...
void three_dimensional_cic_conserves_charge() {
    ...
    constexpr std::size_t nx = 7U, ny = 6U, nz = 5U;
    constexpr double lx = 1.0, ly = 0.8, lz = 0.6;
    std::vector<PicParticle3D> particles(3U);
    particles[0].mass_kg = 1.0;
    particles[0].charge_c = 2.0e-15;
    particles[0].weight = 1.0;
    particles[0].position_m = {0.17, 0.23, 0.31};
    particles[1].mass_kg = 1.0;
    particles[1].charge_c = -1.0e-15;
    particles[1].weight = 2.0;
    particles[1].position_m = {1.11, -0.04, 0.58};
    particles[2].mass_kg = 1.0;
    particles[2].charge_c = 0.5e-15;
    particles[2].weight = 3.0;
    particles[2].position_m = {0.49, 0.79, 0.12};
    const auto rho = deposit_cic_charge_density_3d(particles, nx, ny, nz, lx, ly, lz);
    const double cell =
        (lx / static_cast<double>(nx)) * (ly / static_cast<double>(ny)) * (lz / static_cast<double>(nz));
    const double grid_charge = std::accumulate(rho.begin(), rho.end(), 0.0) * cell;
    double particle_charge = 0.0;
    for (const auto& particle : particles)
        particle_charge += particle.charge_c * particle.weight;
...
void three_dimensional_periodic_poisson() {
    using namespace cfd::particle;
    constexpr std::size_t nx = 8U, ny = 7U, nz = 6U;
    constexpr double lx = 1.0, ly = 0.7, lz = 0.5, epsr = 1.7;
    const double kx = 2.0 * std::numbers::pi / lx;
    const double ky = 2.0 * std::numbers::pi / ly;
    const double kz = 2.0 * std::numbers::pi / lz;
    ...
    for (std::size_t iz = 0; iz < nz; ++iz) {
        ...
        for (std::size_t iy = 0; iy < ny; ++iy) {
            ...
            for (std::size_t ix = 0; ix < nx; ++ix) {
                ...
                const double phi = std::sin(kx * x) * std::cos(ky * y) * std::cos(kz * z);
                const std::size_t idx = (iz * ny + iy) * nx + ix;
...
void charge_conserving_current_3d() {
    ...
    for (std::size_t i = 0; i < before.size(); ++i) {
        ...
        before[i].charge_c = (i % 2U == 0U ? 1.0 : -0.8) * 1.0e-12;
        before[i].weight = 1.0 + 0.1 * static_cast<double>(i);
        before[i].position_m = {0.08 + 0.13 * static_cast<double>(i), 0.11 + 0.09 * static_cast<double>(i),
                                0.07 + 0.05 * static_cast<double>(i)};
        after[i] = before[i];
        after[i].position_m.x += 0.013 + 0.001 * static_cast<double>(i);
        after[i].position_m.y -= 0.009 + 0.002 * static_cast<double>(i);
        after[i].position_m.z += 0.006 + 0.001 * static_cast<double>(i);
    }
    const auto current = deposit_charge_conserving_current_3d(before, after, 8U, 7U, 6U, 1.0, 0.8, 0.6, 2.0e-9);
    require(current.current_x_a_per_m2.size() == 8U * 7U * 6U && current.current_y_a_per_m2.size() == 8U * 7U * 6U &&
                current.current_z_a_per_m2.size() == 8U * 7U * 6U,
            "3-D current deposition returns grid-sized vectors");
...
void electrostatic_pic3d_and_gather() {
    using namespace cfd::particle;
    ElectrostaticPic3DConfig config;
    config.nx = 8U;
    config.ny = 7U;
    config.nz = 6U;
    config.length_x_m = 1.0;
    config.length_y_m = 0.75;
    config.length_z_m = 0.6;
    config.dt_s = 2.0e-6;
    ElectrostaticPic3D solver(config);
    std::vector<PicParticle3D> particles(2U);
    particles[0].mass_kg = 1.0;
    particles[0].charge_c = 1.0e-15;
    particles[0].position_m = {0.23, 0.27, 0.13};
    particles[1].mass_kg = 1.0;
    particles[1].charge_c = -1.0e-15;
    particles[1].position_m = {0.61, 0.52, 0.41};
    ...
    for (double value : solver.electric_y())
        peak_field = std::max(peak_field, std::abs(value));
    for (double value : solver.electric_z())
        peak_field = std::max(peak_field, std::abs(value));
    require(peak_field > 0.0, "3-D electrostatic PIC produces nonzero local field for separated neutral pairs");
    const auto field = solver.gather_electric_field({0.33, 0.29, 0.21});
    require(std::isfinite(field.x) && std::isfinite(field.y) && std::isfinite(field.z),
            "3-D field gather returns finite trilinear values");
    solver.step(3U);
    require(std::abs(solver.time_s() - 6.0e-6) < 1.0e-15, "3-D electrostatic PIC advances time");
}
} // namespace
