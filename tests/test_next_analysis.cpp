...
void sparse_mna() {
    ...
    for (std::size_t i = 0; i < sections; ++i)
        nodes.push_back(ladder.node("n" + std::to_string(i)));
    ...
    Circuit transient;
    const auto in = transient.node("in"), out = transient.node("out");
...
void dynamic_dae_device() {
    ...
    Circuit circuit;
    const auto in = circuit.node("in"), out = circuit.node("out");
    ...
    NewtonConfig forced_sparse;
    forced_sparse.sparse_mna_threshold = 1U;
    forced_sparse.sparse_dense_fallback = false;
    const double dt = 1.0e-4;
    const std::size_t steps = 10U;
    ...
    require(std::abs(transient.back().node_voltage[out] - exact_be) < 2e-8,
            "dynamic DAE backward-Euler charge history");
...
void electrothermal_device() {
    using namespace cfd::circuit;
    Circuit circuit;
    const auto electrical = circuit.node("electrical"), thermal = circuit.node("theta");
    circuit.add_voltage_source("V", electrical, 0, 10.0);
    ElectroThermalDeviceConfig thermal_config;
    thermal_config.ambient_temperature_k = 300.0;
    thermal_config.thermal_resistance_k_per_w = 10.0;
    thermal_config.thermal_capacitance_j_per_k = 0.1;
    circuit.add_electrothermal_device(
        "RHEAT", {electrical, 0}, thermal, thermal_config, [](std::span<const double> voltage, double) {
    ...
    const auto op = circuit.dc_operating_point();
    require(op.converged && std::abs(op.node_voltage[thermal] - 10.0) < 2e-7, "electrothermal DC self-heating balance");
    NewtonConfig sparse;
    sparse.sparse_mna_threshold = 1U;
    sparse.sparse_dense_fallback = false;
    const double dt = 0.1;
    ...
    const double exact = 10.0 * (1.0 - std::pow(1.0 / (1.0 + dt / (thermal_config.thermal_resistance_k_per_w *
                                                                   thermal_config.thermal_capacitance_j_per_k)),
                                                static_cast<double>(steps)));
    require(std::abs(transient.back().node_voltage[thermal] - exact) < 2e-6, "electrothermal thermal-RC transient");
}
...
void adaptive_transient() {
    ...
    AdaptiveTransientConfig controls;
    controls.initial_step = 1.0e-3;
    controls.minimum_step = 1.0e-8;
    controls.maximum_step = 1.0e-3;
    controls.relative_tolerance = 2.0e-5;
    controls.absolute_tolerance = 1.0e-8;
    ...
    require(result.minimum_accepted_step < 0.5 * result.maximum_accepted_step,
            "adaptive transient must use variable time steps");
    require(std::abs(result.points.back().node_voltage[out] - exact) < 3.0e-4, "adaptive RC step response accuracy");
    require(std::abs(result.points.back().time - 5.0e-3) < 1.0e-14,
            "adaptive transient must land on requested stop time");
    ...
    AdaptiveTransientConfig gear;
    gear.method = TransientMethod::bdf2;
    gear.relative_tolerance = 1.0e6;
    gear.absolute_tolerance = 1.0e6;
    gear.minimum_scale = 1.0;
    gear.maximum_scale = 1.0;
    gear.safety_factor = 1.0;
    ...
    AdaptiveTransientConfig variable = controls;
    variable.method = TransientMethod::bdf2;
...
void pss_and_pole_zero() {
    ...
    Circuit periodic;
    const auto in = periodic.node("in"), out = periodic.node("out");
    periodic.add_voltage_source("VS", in, 0, 0.0, Complex{1.0, 0.0}, SineWaveform{0.0, 1.0, f, 0.0, 0.0, 0.0});
    periodic.add_resistor("R", in, out, r);
    periodic.add_capacitor("C", out, 0, c);
    PeriodicSteadyStateConfig pss;
    pss.period_s = 1.0 / f;
    pss.samples_per_period = 128U;
    pss.max_periods = 20U;
    pss.relative_tolerance = 2.0e-4;
    pss.absolute_tolerance = 1.0e-7;
    const auto steady = periodic_steady_state(periodic, pss);
    require(steady.converged && steady.periods < pss.max_periods,
            "RC PSS must converge by cycle-to-cycle shooting residual");
    ...
    require(std::abs(measurement.magnitude - expected) < 2.0e-3,
            "RC PSS amplitude must agree with small-signal solution");

    Circuit lowpass;
    const auto li = lowpass.node("in"), lo = lowpass.node("out");
    lowpass.add_voltage_source("VAC", li, 0, 0.0, Complex{1.0, 0.0});
    lowpass.add_resistor("R", li, lo, r);
    lowpass.add_capacitor("C", lo, 0, c);
    PoleZeroConfig pz;
    pz.start_hz = 1.0;
    pz.stop_hz = 1.0e5;
    pz.samples = 40U;
    pz.denominator_order = 1U;
    pz.numerator_order = 0U;
    ...
    require(std::abs((lp.poles_rad_per_s[0].real() + 1000.0) / 1000.0) < 2.0e-4 &&
                std::abs(lp.poles_rad_per_s[0].imag()) < 1.0e-5,
            "RC low-pass pole location");
    require(lp.relative_rms_fit_error < 1.0e-6, "RC low-pass rational fit accuracy");

    Circuit highpass;
    const auto hi = highpass.node("in"), ho = highpass.node("out");
    highpass.add_voltage_source("VAC", hi, 0, 0.0, Complex{1.0, 0.0});
    highpass.add_capacitor("C", hi, ho, c);
    highpass.add_resistor("R", ho, 0, r);
    pz.numerator_order = 1U;
    const auto hp = pole_zero_analysis(highpass, "out", pz);
...
void transformer_and_lossy_line() {
    using namespace cfd::circuit;
    Circuit transformer;
    const auto primary = transformer.node("p"), secondary = transformer.node("s");
    ...
    require(std::abs(transformer.voltage(op, "p") / transformer.voltage(op, "s") - 2.0) < 1.0e-12,
            "ideal transformer turns ratio");

    constexpr double frequency = 1.0e9;
    const std::array<double, 1> samples{{frequency}};
    Circuit line;
    const auto a = line.node("a"), b = line.node("b");
...
void compact_device_noise() {
    ...
    const auto has_positive = [](const NoiseResult& result, const std::string& prefix) {
        for (const auto& item : result.contributions)
            if (item.source.rfind(prefix, 0) == 0U && item.output_noise_density_v2_per_hz > 0.0)
                return true;
        return false;
    };
    Circuit mos;
    ...
    mos.add_resistor("RD", vdd, drain, 2000.0);
    mos.add_mos_model("NM", {false, 0.7, 1.0e-3, 0.02});
    mos.add_mosfet("M1", drain, gate, 0, 0, "NM");
    const auto mn = mos.output_noise(1.0e3, "d");
    require(has_positive(mn, "M1") && mn.output_noise_density_v2_per_hz > 0.0,
            "MOS channel thermal-noise contribution");

    Circuit jf;
    vdd = jf.node("vdd");
    drain = jf.node("d");
    jf.add_voltage_source("VDD", vdd, 0, 5.0);
    jf.add_resistor("RD", vdd, drain, 1000.0);
    jf.add_jfet_model("NJ", {false, -2.0, 1.0e-3, 0.01});
    jf.add_jfet("J1", drain, 0, 0, "NJ");
    const auto jn = jf.output_noise(1.0e3, "d");
    require(has_positive(jn, "J1"), "JFET channel thermal-noise contribution");

    Circuit bjt;
    vdd = bjt.node("vcc");
    auto vbb = bjt.node("vbb"), base = bjt.node("b"), collector = bjt.node("c");
    bjt.add_voltage_source("VCC", vdd, 0, 5.0);
    bjt.add_voltage_source("VBB", vbb, 0, 5.0);
    bjt.add_resistor("RC", vdd, collector, 1000.0);
    bjt.add_resistor("RB", vbb, base, 100000.0);
    bjt.add_bjt_model("QN", {false, 1e-15, 100.0, 1.0, 300.0});
    bjt.add_bjt("Q1", collector, base, 0, "QN");
    const auto qn = bjt.output_noise(1.0e3, "c");
    require(has_positive(qn, "Q1:collector") && has_positive(qn, "Q1:base"),
            "BJT collector/base shot-noise contributions");
...
RLOAD out 0 1k
    ...
    AdaptiveNetworkSweepConfig sweep_controls;
    sweep_controls.initial_points = 3U;
    sweep_controls.maximum_points = 129U;
    sweep_controls.relative_tolerance = 5.0e-3;
    sweep_controls.absolute_tolerance = 1.0e-6;
    const auto sweep = adaptive_network_sweep(
        1.0e3, 1.0e7, reference,
        [](double frequency) {
            ComplexMatrix matrix(1U);
            const Complex pole{1.0, frequency / 1.0e5};
            matrix(0, 0) = Complex{0.8, 0.0} / pole;
            return matrix;
        },
        sweep_controls);
...
void rf_network_and_peec_extensions() {
    ...
    constexpr std::size_t causal_count = 17U;
    constexpr double df = 1.0e6;
    constexpr double sample_dt = 1.0 / (32.0 * df);
    constexpr double delay = 3.0 * sample_dt;
    for (std::size_t k = 0; k < causal_count; ++k) {
        const double frequency = df * static_cast<double>(k);
        NPortPoint causal;
        causal.frequency_hz = frequency;
        causal.s = ComplexMatrix(1U);
        causal.reference_impedance = {50.0};
        causal.s(0, 0) = std::exp(Complex{0.0, -2.0 * std::numbers::pi * frequency * delay});
        NPortPoint advance = causal;
        advance.s(0, 0) = std::exp(Complex{0.0, 2.0 * std::numbers::pi * frequency * delay});
        causal_points.push_back(std::move(causal));
        advance_points.push_back(std::move(advance));
    }
    ...
    require(std::abs(match.reflection_coefficient) < 1.0e-15 && std::isinf(match.return_loss_db) &&
                std::abs(match.vswr - 1.0) < 1.0e-15,
...
void antenna_optimization() {
    ...
    AntennaOptimizationConfig controls;
    controls.parameter_tolerance = 1.0e-7;
    controls.max_iterations = 96U;
    const auto result = optimize_antenna(
        std::span<const AntennaOptimizationVariable>(&variable, 1U),
...
void multi_wire_mom() {
    ...
    ParallelWireMomConfig config;
    config.frequency_hz = frequency;
    config.quadrature_order = 8U;
    ...
    for (std::size_t i = 0; i < 21U; ++i)
        driven = std::max(driven, std::abs(first.current_a[i]));
    for (std::size_t i = 21U; i < 42U; ++i)
        induced = std::max(induced, std::abs(first.current_a[i]));
    ...
    OrientedWireMomConfig rotated;
    rotated.frequency_hz = frequency;
    rotated.quadrature_order = 8U;
    for (double sign : {-1.0, 1.0}) {
        const WirePoint3 center{sign * 0.05 * wavelength * n.x, sign * 0.05 * wavelength * n.y, 0.0};
        rotated.wires.push_back({{center.x - 0.5 * 0.47 * wavelength * d.x, center.y - 0.5 * 0.47 * wavelength * d.y,
                                  center.z - 0.5 * 0.47 * wavelength * d.z},
                                 {center.x + 0.5 * 0.47 * wavelength * d.x, center.y + 0.5 * 0.47 * wavelength * d.y,
                                  center.z + 0.5 * 0.47 * wavelength * d.z},
                                 0.001 * wavelength,
                                 21U});
    }
    ...
    require(rotation_error / std::max(rotation_reference, 1.0e-30) < 2.0e-10,
            "oriented-wire MoM rotational invariance");
    ...
    require(std::isfinite(std::abs(sport(1, 0))) && std::abs(sport(1, 0)) > 1.0e-8, "two-antenna MoM coupling S21");

    OrientedWireMomConfig line_loaded = rotated;
    line_loaded.feeds.resize(1U);
    line_loaded.transmission_line_loads.push_back(
        {0U, 10U, 1U, 10U, {75.0, 0.0}, {0.2, 2.0 * std::numbers::pi / wavelength}, 0.13 * wavelength, 1, -1});
    ...
    OrientedWireMomConfig ground;
    ground.frequency_hz = frequency;
    ground.quadrature_order = 8U;
    ground.ground_model = ThinWireGroundModel::pec_plane;
    ground.ground_plane_z_m = 0.0;
    ground.wires = {{{-0.235 * wavelength, 0.0, 0.15 * wavelength},
                     {0.235 * wavelength, 0.0, 0.15 * wavelength},
                     0.001 * wavelength,
                     21U}};
    ground.feeds = {{0U, std::numeric_limits<std::size_t>::max(), {1.0, 0.0}}};
    const auto grounded = solve_oriented_thin_wires(ground);
    OrientedWireMomConfig shifted_ground = ground;
    shifted_ground.ground_plane_z_m = 0.7 * wavelength;
    shifted_ground.wires[0].start_m.z += 0.7 * wavelength;
    shifted_ground.wires[0].end_m.z += 0.7 * wavelength;
    ...
    require(std::abs(grounded.feed_impedance_ohm.front() - shifted_grounded.feed_impedance_ohm.front()) /
                    std::max(std::abs(grounded.feed_impedance_ohm.front()), 1.0e-30) <
                2.0e-10,
            "PEC-ground image kernel translation invariance");
    auto free_horizontal = ground;
    free_horizontal.ground_model = ThinWireGroundModel::free_space;
    ...
    OrientedWireMomConfig lossy_wire;
    lossy_wire.frequency_hz = frequency;
    lossy_wire.quadrature_order = 8U;
    OrientedThinWire copper{{0.0, 0.0, -0.235 * wavelength}, {0.0, 0.0, 0.235 * wavelength}, 0.001 * wavelength, 21U};
    copper.conductivity_s_per_m = 5.8e7;
    copper.relative_permeability = 1.0;
    copper.dielectric_loss_ohm_per_m = 0.25;
    lossy_wire.wires = {copper};
    lossy_wire.feeds = {{0U, std::numeric_limits<std::size_t>::max(), {1.0, 0.0}}};
    const auto lossy_result = solve_oriented_thin_wires(lossy_wire);
    require(lossy_result.accepted_power_w > 0.0 && lossy_result.conductor_loss_w > 0.0 &&
                lossy_result.dielectric_loss_w > 0.0,
            "thin-wire distributed conductor/dielectric loss accounting");
    require(lossy_result.radiated_power_w > 0.0 && lossy_result.radiation_efficiency > 0.0 &&
                lossy_result.radiation_efficiency < 1.0,
            "thin-wire radiation efficiency includes distributed losses");

    OrientedWireMomConfig connected;
    connected.frequency_hz = frequency;
    connected.quadrature_order = 8U;
    ...
    OrientedWireMomConfig crossing = rotated;
    crossing.wires = {{{-0.25 * wavelength, 0.0, 0.0}, {0.25 * wavelength, 0.0, 0.0}, 0.001 * wavelength, 21U},
...
int main() {
    try {
        sparse_mna();
        dynamic_dae_device();
        electrothermal_device();
        adaptive_transient();
        pss_and_pole_zero();
        transformer_and_lossy_line();
        compact_device_noise();
        distortion_and_rf_adaptive_tools();
        rf_network_and_peec_extensions();
        antenna_optimization();
        multi_wire_mom();
        std::cout << "next analysis tests passed\n";
        return 0;
    } catch (const std::exception& error) {
        std::cerr << "next analysis test failed: " << error.what() << '\n';
        return 1;
    }
}
